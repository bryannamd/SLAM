...
```
spec/     ★ canonical — 에이전트가 읽고 쓰는 '사실' (루트는 빈 템플릿: TODO를 채운다)
...
examples/
  sip-reminder/    채워진 참조 구현(물 마시기 알림, 오프라인) — 그대로 green
scripts/
...
루트 `spec/`은 **빈 템플릿**이다. 값이 `TODO`라 처음엔 검증이 FAIL하는데, 이건 정상이다 —
`validate.py`가 **채워야 할 필드 목록(체크리스트)**을 그대로 뱉는다. TODO를 다 채우면 PASS로 바뀐다.

```bash
python3 scripts/validate.py            # 루트 빈 템플릿 → 채울 TODO 목록을 FAIL로 출력
python3 scripts/validate.py --selftest      # 대비 계산 자가검증

# 채워진 참조 구현은 --root로 검증(green):
python3 scripts/validate.py --root examples/sip-reminder
python3 scripts/validate.py --root examples/sip-reminder --gate launch
python3 scripts/status.py --root examples/sip-reminder   # 예시 대시보드
```

**새 앱 시작**: 루트 `spec/`의 `TODO`를 실제 값으로 바꾼다(**키 구조 유지, 값만 교체**).
막히면 `examples/sip-reminder/`의 같은 파일을 참고한다. `validate.py`가 남은 자리표시자를 잡아 미완성을 막는다.
